...
class IncomeTaxRates:
    ...
    def _load_tax_data(self, filename: str) -> pd.DataFrame:
        ...
        try:
            ...
            raise FileNotFoundError(f"Tax data file not found: {filename}")
    ...
    def calculate_federal_income_tax_rate(self) -> float:
        ...
        """
        df = self._load_tax_data('data/2024 Federal Income Tax Rates and Brackets.csv')
        brackets = df[
    ...
    def calculate_state_income_tax_rate(self) -> float:
        ...
        Raises:
            ValueError: If no matching tax bracket is found.
        """
        df = self._load_tax_data('data/2024 State Tax Brackets.csv')
        brackets = df[
            (df['state'] == self.state) &
            (df['filing_status'] == self.filing_status) &
            (df['income_ll'] <= self.income)
    ...
    def calculate_federal_ltcg_rate(self) -> float:
        ...
        Raises:
            ValueError: If no matching tax bracket is found.
        """
        df = self._load_tax_data('data/Federal Long Term Capital Gains.csv')
        brackets = df[
            (df['filing_status'] == self.filing_status) &
            (df['income_ll'] <= self.income)
        ].sort_values('income_ll').to_dict('records')
    ...
    def calculate_state_ltcg_rate(self) -> float:
        ...
        Raises:
            FileNotFoundError: If the state LTCG data file is not found.
        """
        try:
            sltcg_df = pd.read_csv('data/State Long Term Capital Gains.csv')
            if self.state in sltcg_df['state'].values:
                return sltcg_df.loc[sltcg_df['state'] == self.state, 'long_term_rate'].values[0]
            return 0.0
        except FileNotFoundError:
            raise FileNotFoundError("State Long Term Capital Gains data file not found")
